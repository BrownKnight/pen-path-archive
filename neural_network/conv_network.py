...
MODEL_PATH = "models/conv_autoencoder.h5"
TEST_SPLIT = 0.1
...
def load_y(path):
    ...
    for index, file_path in enumerate(file_paths):
        with open(file_path) as file:
            ...
            lines = [np.asarray(point.split(','), dtype=int) for point in lines]
...
def create_model():
    ...
    model.compile(optimizer=optimizers.Adam(learning_rate=0.000005), loss=losses.MeanAbsoluteError(),
                  metrics=["accuracy"])
...
def train_model(model: models.Sequential, train_x: np.ndarray, train_y: np.ndarray):
    ...
    history = model.fit(train_x, train_y, batch_size=32, epochs=600, verbose=1, validation_split=TEST_SPLIT,
                        shuffle=True)
